...
fn main() {
    ...
    while !rl.window_should_close() {
        ...
        buffer.x =  std::mem::replace(&mut recv[1], vec![]);
        buffer.y =  std::mem::replace(&mut recv[0], vec![]);
        ...
        d.draw_rectangle(0,0, opt.screen_width, opt.screen_height, Color::BLACK.alpha(0.7));
        let mut last: Option<Vector2>= None;
        for _ in 1..opt.sampling as usize-1 {
            ...
            trail_color.a = ((opt.screen_width * opt.screen_height) as f32 * opt.decay / ((sample.x - previous_sample.x).powi(2) + (sample.y - previous_sample.y).powi(2)) as f32).clamp(0.0, 255.0) as u8 ;
            d.draw_line_ex(sample, previous_sample, point_radius, trail_color);
            //d.draw_circle_v(sample, point_radius, trail_color);
            last = Some(sample)
        };
...
fn show_raw(mut buffer: Buffer) {
    println!("X: {:?}, Samples: {:?}", buffer.x.pop(), buffer.samples_x);
    println!("Y: {:?}, Samples: {:?}", buffer.y.pop(), buffer.samples_y);
